<div class="row">
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> 
            <span class="info-box-icon shadow-sm text-bg-success"> 
                <i class="bi bi-hand-thumbs-up-fill"></i> 
            </span>
            <div class="info-box-content"> 
                <span class="info-box-text">Diterima</span> 
                <span class="info-box-number"> 190 <small>Siswa</small> </span> 
            </div>
        </div>
    </div>
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> 
            <span class="info-box-icon shadow-sm text-bg-danger"> 
                <i class="bi bi-x-circle-fill"></i> 
            </span>
            <div class="info-box-content"> 
                <span class="info-box-text">Ditolak</span> 
                <span class="info-box-number"> 190 <small>Siswa</small> </span> 
            </div>
        </div>
    </div>
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> 
            <span class="info-box-icon shadow-sm text-bg-warning"> 
                <i class="bi bi-hourglass-split text-white"></i> 
            </span>
            <div class="info-box-content"> 
                <span class="info-box-text">Dicadangkan</span> 
                <span class="info-box-number"> 190 <small>Siswa</small> </span> 
            </div>
        </div>
    </div>
    
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> 
            <span class="info-box-icon shadow-sm text-bg-info"> 
                <i class="bi bi-question-circle-fill text-white"></i> 
            </span>
            <div class="info-box-content"> 
                <span class="info-box-text">Belum Diverifikasi</span> 
                <span class="info-box-number"> 190 <small>Siswa</small> </span> 
            </div>
        </div>
    </div>
</div>
